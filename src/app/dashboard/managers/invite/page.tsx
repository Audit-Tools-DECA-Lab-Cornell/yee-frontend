		...
		return (
			<Card className="rounded-[1.75rem] border-slate-200/80 bg-white shadow-sm">
				<CardHeader>
					...
					<CardDescription>Only the primary manager can invite additional managers into this organization.</CardDescription>
				</CardHeader>
				<CardContent>
		...
		<div className="space-y-6">
			<Card className="rounded-[1.75rem] border-slate-200/80 bg-white shadow-sm">
				<CardHeader>
					...
					<CardDescription className="max-w-2xl leading-6">
						Invite an additional manager into your organization. They will join this same account and gain manager access after accepting.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-6">
					...
					{invite ? (
						<div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5 text-sm leading-7 text-emerald-900">
							...
							<p className="mt-2 break-all rounded-2xl bg-white px-4 py-3 font-mono text-xs text-slate-700">{invite.invite_url}</p>
							<p className="mt-2 text-emerald-800">Expires at: {new Date(invite.expires_at).toLocaleString()}</p>
						</div>
					) : null}
				</CardContent>
			</Card>
			...
			<Card className="rounded-[1.75rem] border-slate-200/80 bg-white shadow-sm">
				<CardHeader>
					...
					<CardDescription>Track pending, accepted, and expired manager invitations for this organization.</CardDescription>
				</CardHeader>
							...
							<tbody>
								{invites.map(row => (
									<tr key={row.id} className="border-b border-slate-100 last:border-0">
										...
										<td className="py-4 pr-4 text-slate-600">{row.created_at ? new Date(row.created_at).toLocaleString() : "-"}</td>
										<td className="py-4 pr-4 text-slate-600">{new Date(row.expires_at).toLocaleString()}</td>
										<td className="py-4">
