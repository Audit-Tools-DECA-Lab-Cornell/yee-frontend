...
export async function fetchAuditState(placeId: string): Promise<YeeAuditState> {
  const response = await fetch(
    `/api/yee/places/${encodeURIComponent(placeId)}/audit-state`,
    { cache: "no-store" }
  );
...
export async function fetchSubmission(submissionId: string): Promise<YeeSubmissionRecord> {
  const response = await fetch(
    `/api/yee/audits/${encodeURIComponent(submissionId)}`,
    { cache: "no-store" }
  );
  return readJsonOrThrow<YeeSubmissionRecord>(response);
}

export async function fetchManagerAuditEditState(
  auditId: string
): Promise<ManagerAuditEditState> {
  const response = await fetch(
    `/api/dashboard/audits/${encodeURIComponent(auditId)}/edit`,
    { cache: "no-store" }
  );
