...
export function SiteCopyAdminPanel() {
	...
	const applyVersions = React.useCallback((rows: SiteCopyVersionRecord[]) => {
		setError(null);
		setVersions(rows);
	...
	const loadAll = React.useCallback(async () => {
		if (!session) return;
		try {
			applyVersions(await fetchSiteCopyVersions(session));
		} catch (err) {
			setError(err instanceof Error ? err.message : "Could not load site copy versions.");
		} finally {
			setLoading(false);
		}
	}, [applyVersions, session]);

	React.useEffect(() => {
		// Inline promise chain (not `void loadAll()`) so every setState happens
		// in an async callback, and the fetch is cancelled on unmount.
		if (!session) return;
		let cancelled = false;
		fetchSiteCopyVersions(session)
			.then(rows => {
				if (!cancelled) applyVersions(rows);
			})
			.catch((err: unknown) => {
				if (!cancelled) setError(err instanceof Error ? err.message : "Could not load site copy versions.");
			})
			.finally(() => {
