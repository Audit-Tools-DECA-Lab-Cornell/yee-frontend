...
export function PlaceProfileForm({
	...
	React.useEffect(() => {
		...
		const timeoutId = window.setTimeout(async () => {
			// The too-short reset also runs inside the debounce so the effect
	...
	// Reset the map preview during render when its inputs change, instead of
	// in an effect, avoiding a cascading re-render.
	const mapUrlsKey = `${staticMapUrl ?? ""}|${fallbackStaticMapUrl ?? ""}`;
	const [prevMapUrlsKey, setPrevMapUrlsKey] = React.useState(mapUrlsKey);
	if (mapUrlsKey !== prevMapUrlsKey) {
		setPrevMapUrlsKey(mapUrlsKey);
