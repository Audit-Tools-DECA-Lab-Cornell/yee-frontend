...
export function createInstrumentVersion(session: FrontendSession, payload: InstrumentCreatePayload, activate = true) {
	return authedFetch<InstrumentVersionRecord>(
...
export function createSiteCopyVersion(
	...
	return authedFetch<SiteCopyVersionRecord>(`/api/admin/site-copy?activate=${activate ? "true" : "false"}`, session, {
		method: "POST",
		body: payload
	});
}

export function updateSiteCopyStatus(session: FrontendSession, copyId: string, payload: { is_active: boolean }) {
	return authedFetch<SiteCopyVersionRecord>(`/api/admin/site-copy/${encodeURIComponent(copyId)}`, session, {
		method: "PATCH",
		body: payload
	});
}

export function approveUser(session: FrontendSession, payload: { user_id: string; account_id?: string }) {
	return authedFetch<UserRecord>("/api/dashboard/users", session, {
		method: "POST",
		body: payload
	});
...
export function updateProject(session: FrontendSession, projectId: string, payload: ProjectProfilePayload) {
	return authedFetch<ProjectRecord>(`/api/dashboard/projects/${encodeURIComponent(projectId)}`, session, {
		method: "PATCH",
		body: payload
	});
}
...
export function updatePlace(session: FrontendSession, placeId: string, payload: PlaceProfilePayload) {
	return authedFetch<PlaceRecord>(`/api/dashboard/places/${encodeURIComponent(placeId)}`, session, {
		method: "PATCH",
		body: payload
	});
}
...
export function removeManager(session: FrontendSession, managerProfileId: string) {
	return authedFetch<null>(`/api/dashboard/managers/${encodeURIComponent(managerProfileId)}`, session, {
		method: "DELETE"
	});
}
...
export function revokeManagerInvite(session: FrontendSession, inviteId: string) {
	return authedFetch<null>(`/api/dashboard/manager-invites/${encodeURIComponent(inviteId)}`, session, {
		method: "DELETE"
	});
}
