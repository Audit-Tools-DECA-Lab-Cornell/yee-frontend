			...
			<CardContent className="space-y-6">
							...
							{rows.map(row => {
								...
								return (
									<tr key={row.domain} className="border-b border-slate-100 last:border-0">
										...
										<td className="px-3 py-4 text-center text-slate-600">
											{rawPercentage.toFixed(0)}% ({row.rawScore}/{rawMax})
										</td>
										<td className="px-3 py-4 text-center text-slate-600">
											{row.weightedScore} / {weightedMax}
										</td>
										<td className="px-3 py-4 text-center text-slate-600">
											{weightedPercentage.toFixed(0)}% ({row.weightedScore}/{weightedMax})
										</td>
									</tr>
								);
							})}
				...
				<div className="grid gap-4 md:grid-cols-2 report-no-break report-print-stack">
					<div className="rounded-lg bg-slate-50 p-4 report-no-break">
						...
						<p className="mt-2 text-2xl font-semibold text-slate-950">
							...
							<span className="text-lg text-slate-600">({totalRawPercentage.toFixed(0)}%)</span>
						</p>
						<p className="mt-2 text-xs text-slate-500">
							This percentage shows how much of the available raw score was achieved across the full
							audit.
						</p>
					</div>
					<div className="rounded-lg bg-emerald-100/80 p-4 report-no-break">
						...
						<p className="mt-2 text-2xl font-semibold text-emerald-900">
							...
							<span className="text-lg text-emerald-700">({totalYouthPercentage.toFixed(0)}%)</span>
						</p>
						<p className="mt-2 text-xs text-emerald-700/80">
							This Youth Weighted maximum is based on normalized domain weights and each domain&apos;s
				...
				<div className="rounded-lg border border-slate-200 bg-[#f8fbf9] p-4 text-sm text-slate-600 report-no-break">
					...
					<p className="mt-2">
						...
						much of that available score was reached. Raw and Youth Weighted percentages are shown
						separately because they answer slightly different questions about the same audit.
					</p>
							...
							{rows.map(row => {
								...
								return (
									<div
										...
										<div className="space-y-1">
											<p className={`text-sm font-semibold ${palette.text}`}>{row.label}</p>
											<p className={`mt-1 text-xs ${youthPalette.text}`}>
					...
					<div className="grid gap-4 md:grid-cols-2 report-no-break report-print-stack">
						<div className="rounded-lg border border-slate-200 bg-slate-50 p-4 report-no-break">
							...
							<p className="mt-2 text-sm text-slate-600">
								Highest:{" "}
								<span className="font-medium text-slate-900">{rawExtremes.highest.row.label}</span> (
								{rawExtremes.highest.percentage.toFixed(0)}%)
							</p>
							<p className="mt-1 text-sm text-slate-600">
								Lowest:{" "}
								<span className="font-medium text-slate-900">{rawExtremes.lowest.row.label}</span> (
