...
test.describe("@manager projects list + create form", () => {
	test("projects list page shows seeded projects", async ({ page }) => {
		...
		await expect(
			page.getByRole("link", { name: "Youth Enabling Environments Baseline 2026" }).first()
		).toBeVisible({ timeout: 15_000 });
		await expect(
			page.getByRole("link", { name: "Community Amenities Follow-up" }).first()
		).toBeVisible();
	...
	test("create project form loads at /dashboard/projects/new", async ({ page }) => {
		await loginAsManager(page);
