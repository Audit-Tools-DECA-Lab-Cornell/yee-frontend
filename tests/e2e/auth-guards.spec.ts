...
//   /admin -> [ADMIN], /dashboard -> [MANAGER], /my-dashboard -> [AUDITOR, MANAGER]
...
test.describe("@auth route guards + negative auth", () => {
	test("unauthenticated visits to protected routes redirect to /login", async ({ page }) => {
		for (const route of ["/dashboard", "/my-dashboard", "/admin"]) {
	...
	test("auditor is redirected away from the manager /dashboard to /my-dashboard", async ({ page }) => {
		await loginAsAuditor(page);
		await page.goto("/dashboard");
		await expect(page).toHaveURL(/\/my-dashboard(\/|$|\?)/);
	});

	test("manager is redirected away from /admin to /dashboard", async ({ page }) => {
	...
	test("bad credentials show an error and stay on /login", async ({ page }) => {
		...
		await expect(
			page.getByText(/invalid|incorrect|failed|credential|password|not verified/i).first()
